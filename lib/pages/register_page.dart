...
class _RegisterPageState extends State<RegisterPage> {
  ...
  // === List Pekerjaan ===
  final List<Map<String, dynamic>> _listPekerjaan = [
    {"id": 1, "nama": "ASN/PNS/TNI/Polri"},
    {"id": 2, "nama": "Mahasiswa/Pelajar"},
    {"id": 3, "nama": "Wiraswasta"},
    {"id": 4, "nama": "Karyawan BUMN/BUMD"},
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _buildPasswordField(controller.password, "Password",
                        isConfirm: false),
                    _buildPasswordField(
                        controller.confirmPassword, "Konfirmasi Password",
                        isConfirm: true),
                    ...
                    _buildField(controller.alamat, "Alamat"),

                    // === ComboBox Pekerjaan ===
                    Padding(
                      padding: const EdgeInsets.only(bottom: 12),
                      child: DropdownButtonFormField<int>(
                        value: controller.selectedPekerjaanId,
                        decoration: InputDecoration(
                          labelText: "Pekerjaan",
                          border: const OutlineInputBorder(),
                          errorText: _errorMessages["Pekerjaan"],
                        ),
                        items: _listPekerjaan.map((pekerjaan) {
                          return DropdownMenuItem<int>(
                            value: pekerjaan["id"],
                            child: Text(pekerjaan["nama"]),
                          );
                        }).toList(),
                        onChanged: (value) {
                          setState(() {
                            controller.setSelectedPekerjaan(value);
                          });
                        },
                      ),
                    ),
  ...
  void _validateField(TextEditingController c, String label) {
    ...
    switch (label) {
      ...
      case "No KTP / SIM / KITA":
        _errorMessages[label] =
        text.length == 16 ? null : "KTP/SIM/KITA harus 16 karakter";
        break;
      case "Nomor BPJS Kesehatan":
        _errorMessages[label] =
        text.length == 13 ? null : "BPJS harus 13 digit";
        break;
      case "Alamat":
        _errorMessages[label] = text.isEmpty ? "Alamat wajib diisi" : null;
        break;
      case "Nomor Handphone":
        _errorMessages[label] =
        RegExp(r"^0\d{9,14}$").hasMatch(text) ? null : "Format nomor tidak valid";
        break;
      case "Email":
        _errorMessages[label] =
      ...
      case "Konfirmasi Password":
        _errorMessages[label] =
        text == controller.password.text ? null : "Password tidak cocok";
        break;
      default:
